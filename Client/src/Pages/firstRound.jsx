...
const Interview = () => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        socketRef.current = new WebSocket("wss://api.deepgram.com/v1/listen?model=nova-2&smart_format=true&interim_results=false", [
          "token",
          "c9a027ca53eca26716be35fd233e1c33e11e0867",
        ]);
  ...
  const handleGenerateNextQuestion = async (transcript) => {
    if (questionCountRef.current < 10) {
      ...
      const completionMessage = "Your interview has been completed. The next round starts now.";
      ...
      setTimeout(() => {
        navigate("/technicalPracticeSimulation");
      }, 5000);
    }
  };
  ...
  const handleNextClick = async () => {
    if (!isInterviewComplete) {
      ...
      setTimer(90);
      setIsTimerRunning(false);
      await handleGenerateNextQuestion(currentAnswer);
    }
  };
  ...
  }, [currentAnswer, currentQuestion, questionCount, user.name]);
  ...
  useEffect(() => {
    if (questionCount >= 10 && !isInterviewComplete) {
      console.log("Question count reached 10, ending interview");
      const completionMessage = "Your interview has been completed. The next round starts now.";
      setCurrentQuestion(completionMessage);
      setIsInterviewComplete(true);
      stopRecording();
  ...
  }, [questionCount, stopRecording, isInterviewComplete, speakText, navigate]);
    ...
    <div className="flex h-screen overflow-y-hidden regular3 ">
      ...
      <div className="w-[30%] flex flex-col justify-between my-8 mr-10">
        ...
        >
          {interviewData.map((item, index) => (
            <div key={index} className="mb-4">
              ...
              <div className="flex flex-col justify-start">
                <p className="text-[10px] text-white mb-2">{user.name}</p>
                <motion.div
                  key={`answer-${index}`}
          ...
          {currentAnswer && (
            <div className="flex flex-col justify-start">
              <p className="text-sm text-white mb-2">{user.name}</p>
              <motion.div
